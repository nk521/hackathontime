...
for x in range(50):
	...
	u = User.objects.create_user(username=passs + ''.join(random.choices(alpha_numeric, k=5)), first_name=''.join(random.choices(alpha_numeric, k=10)), last_name=''.join(random.choices(alpha_numeric, k=5)), email=''.join(random.choices(alpha_numeric, k=10))+"@abcd.com", password=passs)
	u.profile.gender = random.choice("MFO")
	u.profile.bio = ''.join([''.join(random.choices(alpha_numeric,k=20)) for _ in range(10)])
