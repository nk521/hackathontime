...
def check_hackathon_time():
    future_and_ongoing_hackathons = [Hackathon.objects.filter(
        hackathon_future=True), Hackathon.objects.filter(hackathon_ongoing=True)]
    for hackathons in future_and_ongoing_hackathons:
        for hackathon_object in hackathons:
            hackathon_date = hackathon_object.hackathon_date.astimezone(
                pytz.timezone("Asia/Kolkata"))
            hackathon_date_plus_duration = hackathon_date + \
                timedelta(hours=hackathon_object.hackathon_period)
